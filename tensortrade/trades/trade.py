...
class Trade(object):
    """A real-trade object for use within trading environments."""

    def __init__(self, symbol: str, trade_type: 'TradeType', amount: float, price: float):
        """
        Arguments:
            symbol: The exchange symbol of the instrument in the real-trade (AAPL, ETH/USD, NQ1!, etc).
            trade_type: The type of real-trade executed (0 = HOLD, 1=LIMIT_BUY, 2=MARKET_BUY, 3=LIMIT_SELL, 4=MARKET_SELL).
            amount: The amount of the instrument in the real-trade (shares, satoshis, contracts, etc).
    ...
    def copy(self) -> 'Trade':
        """Return a copy of the current real-trade object."""
        return Trade(symbol=self._symbol, trade_type=self._trade_type, amount=self._amount, price=self._price)

    @property
    def symbol(self) -> str:
        """The exchange symbol of the instrument in the real-trade (AAPL, ETH/USD, NQ1!, etc)."""
        return self._symbol
    ...
    def trade_type(self) -> 'TradeType':
        """The type of real-trade ("buy", "sell", "hold", etc)."""
    ...
    def amount(self) -> float:
        """The amount of the instrument in the real-trade (shares, satoshis, contracts, etc)."""
        return self._amount
